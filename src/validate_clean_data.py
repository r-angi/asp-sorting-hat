...
def _validate_crews_schema_and_values(crews: pl.DataFrame, report: ValidationReport) -> None:
    ...
    illegal_blank_center = crews.filter(
        center_blank & ~(pl.col("role").is_in(["Adult", "Young Adult"]) & crew_blank)
    )
    ...
    non_blank_crew = crews.filter(
        pl.col("Crew").is_not_null() & (pl.col("Crew").cast(pl.Utf8).str.strip_chars() != "")
    )
    ...
    dupes = (
        crews.filter(pl.col("name").is_not_null())
        .group_by("name")
        .len()
        .filter(pl.col("len") > 1)
    )
    ...
    if "history" in crews.columns:
        ...
        empty_hist = adults_ya.filter(
            pl.col("history").is_null()
            | (pl.col("history").cast(pl.Utf8).str.strip_chars() == "")
        )
        if len(empty_hist) > 0:
            samples = empty_hist.select("name").head(5).to_dicts()
            report.warnings.append(
                "Crews CSV has Adult/Young Adult rows with empty history; "
                f"adult vet/new balancing may be weaker on those crews; samples: {samples}"
            )
        bad_hist = adults_ya.filter(
            pl.col("history").cast(pl.Utf8).str.strip_chars() != ""
        ).filter(~pl.col("history").cast(pl.Utf8).str.strip_chars().is_in(list(ALLOWED_HISTORY)))
        if len(bad_hist) > 0:
            vals = bad_hist["history"].unique().to_list()
            report.errors.append(
                f"Crews CSV invalid history values for Adult/Young Adult {vals} (allowed: {sorted(ALLOWED_HISTORY)})"
            )

    if "gender" in crews.columns:
        adults_ya_g = crews.filter(pl.col("role").is_in(["Adult", "Young Adult"]))
        bad_gender = adults_ya_g.filter(
            pl.col("gender").cast(pl.Utf8).str.strip_chars() != ""
        ).filter(~pl.col("gender").cast(pl.Utf8).str.strip_chars().is_in(list(CREW_GENDER_LONG_TO_SHORT)))
        if len(bad_gender) > 0:
            vals = bad_gender["gender"].unique().to_list()
            report.errors.append(
                f"Crews CSV invalid gender values for Adult/Young Adult {vals} "
                f"(allowed: {sorted(CREW_GENDER_LONG_TO_SHORT)})"
            )

    for opt_col in CREWS_OPTIONAL_COLUMNS:
        if opt_col not in crews.columns:
            continue
        stray = crews.filter(pl.col("role") == "Youth").filter(
            pl.col(opt_col).cast(pl.Utf8).str.strip_chars() != ""
        )
        if len(stray) > 0:
            report.warnings.append(
...
def _validate_preassigned_adult_rules(crews: pl.DataFrame, report: ValidationReport) -> None:
    ...
    for row in grouped.iter_rows(named=True):
        ...
        if adult_count < 1:
            report.errors.append(
                f"Pre-assigned crew {center_name}/{crew_name} has no Adult (driver); "
...
def _validate_buddies_schema_and_values(buddies: pl.DataFrame, report: ValidationReport) -> None:
    ...
    for row in buddies.iter_rows(named=True):
        ...
        if len(non_empty) != len(set(non_empty)):
            ...
            report.errors.append(
                f"{normalize_person_name(str(row['name']))} has duplicate friend pick(s) "
                f"across first/second/third choice: {duplicated}"
            )
    ...
    for row in buddies.iter_rows(named=True):
        ...
        for choice_key in CHOICE_FIELDS:
            ...
            if choice not in youth_set and choice not in adult_set:
                ...
                report.errors.append(
                    f"{youth}'s {choice_key} {choice!r} is not a youth in buddies nor an Adult/Young Adult on crews"
                    f"{hint}"
                )
