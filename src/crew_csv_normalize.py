...
def _ensure_core_aliases(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if 'role' not in by_lower and 'adult/ya' in by_lower:
        ...
        out = out.with_columns(
            pl.when(pl.col(src).cast(pl.Utf8).str.strip_chars() == 'YA').then(pl.lit('Young Adult')).otherwise(pl.col(src)).alias('role')
        ).drop(src)
    return out
...
def normalize_crews_dataframe(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if not required.issubset(present):
        raise ValueError(f'Could not resolve required columns {BASE_CREW_COLUMNS}; mapped to {sorted(present)} from headers {work.columns!r}')
    ...
    out = out.with_columns(
        pl.when(pl.col('role').cast(pl.Utf8).str.strip_chars() == 'YA').then(pl.lit('Young Adult')).otherwise(pl.col('role')).alias('role'),
    )
    ...
    out = out.with_columns([pl.when(adult_mask).then(pl.col(col)).otherwise(pl.lit('')).alias(col) for col in ADULT_DETAIL_COLUMNS])
