    ...
    for cluster_id, members in cluster_members.items():
        ...
        for name in members:
            for center in centers:
                if solver.Value(person_center[name, center.name]) == 1:
...
def analyze_clusters(
    ...
    cohesion = calculate_cluster_cohesion(clusters, solver, person_center, centers)
    visualize_cluster_distribution(
        clusters, cohesion, centers, solver, person_center,
        f"{output_dir}/cluster_analysis.png"
    )
    ...
    for cluster_id, data in sorted(cohesion.items(), key=lambda x: x[1]["size"], reverse=True):
        print(f'  {cluster_id}: {data["size"]} members, '
              f'cohesion={data["cohesion_score"]:.1%}, '
              f'distribution={data["center_distribution"]}')
